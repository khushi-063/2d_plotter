...
L1 = 0.30   # upper arm — FIXED (was swapped)
...
# ════════════════════════════════════════════════════════════
# CALIBRATED CONSTRAINTS — paste fresh from calibrate_workspace.py
# ════════════════════════════════════════════════════════════
...
MAX_RADIUS_FIT = min(
    (WORKSPACE_WIDTH  / 2.0) - WORKSPACE_MARGIN,
    (WORKSPACE_HEIGHT / 2.0) - WORKSPACE_MARGIN,
)
RADIUS   = min(0.03, MAX_RADIUS_FIT)
...
def setup(serial, gear, tuning, name, start_angle_deg=0.0, sign=1.0):
    ...
    # FIXED: normal convention — zero = encoder pos when joint = 0°
...
class JointLogger(threading.Thread):
    ...
    def save_plot(self, path, cx, cy, radius):
        try:
        ...
        phase_colors = {'idle':'gray','move_to_start':'orange','circle':'tab:blue','return':'tab:green'}
        ...
        thr = np.linspace(0, 2*np.pi, 200)
        ax[1,1].plot(cx+radius*np.cos(thr), cy+radius*np.sin(thr), 'k:', alpha=0.5)
...
def trace_circle(joints, logger, cx, cy, radius):
    ax_s, zero_s, gear_s, sign_s, ax_e, zero_e, gear_e, sign_e = joints
    dt = LOOP_TIME / N_POINTS
    for loop in range(N_LOOPS):
        for i in range(N_POINTS + 1):
            ...
            logger.set_cmd(th1, th2, 'circle')
            move_rad(ax_s, zero_s, gear_s, th1, TH1_MIN_DEG, TH1_MAX_DEG, 'Shoulder', sign_s)
            move_rad(ax_e, zero_e, gear_e, th2, TH2_MIN_DEG, TH2_MAX_DEG, 'Elbow', sign_e)
            if i % 45 == 0:
                print(f'  loop {loop+1}/{N_LOOPS}  θ={math.degrees(theta):6.1f}°  '
                      f'xy=({x*100:+5.2f}, {y*100:+5.2f}) cm')
            time.sleep(dt)


def precheck_circle(cx, cy, radius):
    print(f'\nPre-checking circle:')
    print(f'  center ({cx*100:+.2f}, {cy*100:+.2f}) cm, radius {radius*100:.2f} cm')
    ...
    for i in range(N_POINTS):
        ...
        try:
            ...
            bad_reach.append((i, x, y, str(e))); continue
        worst_th2 = max(worst_th2, math.degrees(th2))
        try: check_joint_limits(th1, th2)
        except ValueError as e: bad_joint.append((i, x, y, str(e)))
        try: check_workspace_bounds(x, y)
        except ValueError as e: bad_ws.append((i, x, y, str(e)))
    if bad_reach or bad_joint or bad_ws:
        ...
        return False
    print(f'  ✓ all {N_POINTS} points OK')
...
# TUNING REPORT
# ════════════════════════════════════════════════════════════
def tuning_report(samples, txt_path, cx, cy, radius):
    import numpy as np
    ...
    lines = ['═'*47, '  TUNING REPORT', '═'*47,
             f'  Circle: ({cx*100:+.2f}, {cy*100:+.2f}) cm  r={radius*100:.2f} cm',
             f'  Duration: {t[-1]:.2f}s   Samples: {len(samples)}', '']
    for ph in ['move_to_start','circle','return']:
    ...
    cm = np.array([p=='circle' for p in phases])
    if cm.any():
...
# MAIN
# ════════════════════════════════════════════════════════════
print('═══ Circle in Calibrated Workspace ═══')
...
print(f'  Max radius: {MAX_RADIUS_FIT*100:.2f} cm  |  Chosen: {RADIUS*100:.2f} cm')
print(f'  Joint limits: θ1 [{TH1_MIN_DEG:+.2f}, {TH1_MAX_DEG:+.2f}]°, '
      f'θ2 [{TH2_MIN_DEG:+.2f}, {TH2_MAX_DEG:+.2f}]°')

if not precheck_circle(CENTER_X, CENTER_Y, RADIUS):
    print('\n❌ Circle does not fit.'); sys.exit(1)
...
try:
    ...
    start_y = CENTER_Y
    print(f'\n[Phase 1/3] Move to circle start ({start_x*100:+.2f}, {start_y*100:+.2f}) cm...')
    ...
    print(f'\n[Phase 2/3] Tracing {N_LOOPS} circle(s)...')
    trace_circle(joints, logger, CENTER_X, CENTER_Y, RADIUS)
    print('  ✓ Done.')
    time.sleep(SETTLE_TIME)
    ...
    try:
        ...
        time.sleep(SETTLE_TIME)
        fx, fy = forward_kinematics(get_angle_rad(ax_s,zero_s,gear_s),
                                    get_angle_rad(ax_e,zero_e,gear_e))
    ...
    logger.save_plot(LOG_PNG, CENTER_X*100, CENTER_Y*100, RADIUS*100)
    tuning_report(logger.samples, TUNING_TXT, CENTER_X, CENTER_Y, RADIUS)
    print('\nDone.')